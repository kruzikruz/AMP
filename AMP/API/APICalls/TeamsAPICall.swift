...
class TeamsAPICall: ObservableObject {
    @Published var teams: [Team_API_DATA] = []
    
    // MARK: - Teams
    
    func getTeams() {
        var allTeams: [Team_API_DATA] = []
    ...
    private func fetchTeams(page: Int, completion: @escaping ([Team_API_DATA], Int) -> Void) {
        ...
        let dataTask = URLSession.shared.dataTask(with: urlRequest) { (data, response, error) in
            ...
            if response.statusCode == 200 {
                ...
                do {
                    let decodedTeams = try JSONDecoder().decode([Team_API_DATA].self, from: data)
